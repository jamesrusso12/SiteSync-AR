...
**Gotcha — `AssetImportTask` (Python) on PNGs crashes the UE 5.6 Mac editor.** Tried to pre-import the SiteIQ logos (`website/assets/*.png`) into `/Game/User_Interface/Branding/` via `AssetToolsHelpers.get_asset_tools().import_asset_tasks([...])` over MCP `execute_python` — the editor hard-crashed reproducibly (twice), even with a lean import (no property edits). Also: the MCP TCP server answers ~15s after launch but the editor isn't fully initialized then — wait + verify `get_editor_world().get_name()` returns before driving it. **For texture imports on Mac, use Content Browser drag-drop (GUI) instead of the Python import task.** The legacy `Build/IOS/Resources/Graphics/Icon*.png` files were removed this session (icon now comes from `Assets.xcassets`).

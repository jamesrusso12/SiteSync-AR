...
2. ~~**Formspree** — replace `YOUR_FORM_ID`~~ ✅ **Wired 2026-05-31** to Formspree endpoint **`mpqngdpe`** ("Join the waitlist" form, under James's personal portfolio Formspree account as a temp recipient). Live page POSTs correctly (endpoint GET returns 405 = POST-only, exists). Used the existing hardened inline AJAX handler — deliberately NOT the `@formspree/ajax` CDN lib (would require widening CSP to allow `unpkg.com` + break the single-file/no-deps principle). The JS `FORM_PLACEHOLDER='YOUR_FORM_ID'` sentinel stays as-is (do NOT replace it — it's the "is configured?" check). The endpoint ID is permanent; **moving to a business-owned Formspree account + `hello@siteiq.app` recipient is delegated to Cole** and only needs the recipient changed Formspree-side (page untouched). Personal email is deliberately NOT placed in the page (public-page spam risk). Note: first submission to a new Formspree form may need a one-time confirmation click from the recipient inbox before it forwards.
...
- **2026-05-21 — cut/fill HUD numbers never settle** — ✅ **RESOLVED 2026-05-28 (`cbb688a`)**. Was: `BP_Foundation` recalc'd `CalculateCutFillVolumes` at 10 Hz against the live (still-scanning) LiDAR mesh, so the readout chased a moving input. Fix: collapsed the recalc body into a `DoRecalculate` event, removed the loop-back wire, BeginPlay now recalcs ONCE after a 1.0s settle. Added a `Recalculate` button on `WBP_VolumeReadout` for on-demand refresh after scanning more terrain. Device-validated 2026-05-28 — numbers hold steady, button refreshes correctly, reset + no-slab paths safe. See `decisions.md 2026-05-21` (resolution note appended).
...
`Content/AR_SiteAnalysis/DatasmithAssets/` — anchor with `.gitkeep`. All imported BIM `.uasset` content goes under here, one subfolder per source model (e.g. `DatasmithAssets/SampleOffice/`, `DatasmithAssets/SiteX/`). Datasmith importer creates `Geometries/`, `Materials/`, `Textures/` subfolders automatically per import.

`BIM_Source/` (repo root, NOT under `Content/`) — raw BIM source files: Rhino `.3dm` models and exported `.udatasmith` bundles, one subfolder per model. Kept outside `Content/` so they are never cooked into the iOS `.ipa`; tracked via Git LFS (`.3dm`, `.udatasmith`, `.udsmesh` per `.gitattributes`) so re-imports/re-cooks are reproducible from either machine.
